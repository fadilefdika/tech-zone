...
import { Product, Report } from '@/types/types';
...
const FiltersProduct = <T extends { date?: string; status: string; category?: string }>({ isDate = false, titleStatus, optionsStatus, titleCategory, optionsCategory = [], data = [], setFilteredData }: FiltersProps<T>) => {
  ...
  const handleFilterChange = () => {
    ...
    const filtered = data.filter((item) => {
      ...
      const isCategoryMatch = category === 'SEMUA' || !category || item.categoryName.toLowerCase() === category.toLowerCase();
