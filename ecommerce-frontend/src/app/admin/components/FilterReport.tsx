...
const FiltersReport: React.FC<FiltersProps> = ({ isDate = false, titleStatus, optionsStatus, dummyData = [], setFilteredData }) => {
  ...
  const handleFilterChange = () => {
    ...
    const filtered = dummyData.filter((item) => {
      ...
      const itemDateParsed = new Date(itemDate);

      const isDateInRange = !isDate || ((!startDate || itemDateParsed >= new Date(startDate)) && (!endDate || itemDateParsed <= new Date(endDate)));
